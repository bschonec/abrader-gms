...
Puppet::Type.type(:git_integration).provide(:gitlab) do
  ...
  def wiki_page_events=(value)
    Puppet.debug("YYYY: SETTER #__method__ #{value}.")
    do_the_needful('wiki_page_events', resource[:wiki_page_events])
  end
  ...
  def get_the_needful(param)
    ...
    Puppet.debug("YYYY: getting CURRENT value #{param}: #{a} :ZZZZ")
